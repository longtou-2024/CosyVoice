...
class WebDataList(IterableDataset):
    ...
    def __init__(self, recipe_names, mode="train", cache_size=0, from_mount=False, from_prod=False):
        #self.sampler = DistributedSampler(shuffle, partition)
        self.epoch = None

        # 1) mix ko with foreign
        #datasets_ko = []
        #datasets_other = []
        #for name in recipe_names:
        #    if name in ("emilia_en", "emilia_zh"):
        #        datasets_other.append(build_wds(name, mode=mode, cache_size=cache_size, from_mount=from_mount, from_prod=from_prod))
        #    else:
        #        datasets_ko.append(build_wds(name, mode=mode, cache_size=cache_size, from_mount=from_mount, from_prod=from_prod))
        #dataset_ko = wds.RoundRobin(datasets_ko, longest=True)

        #if len(datasets_other) > 0:
        #    dataset_other = wds.RoundRobin(datasets_other, longest=True)
        ...
        ds1, ds5 = [],[]
        for name in recipe_names:
            if name in ("commbooks", "commbooks_speaking_rate", "commbooks_tone", "literature", "literature_speaking_rate", "literature_tone", "skt_emotion_large", "mediazen_teen", "mediazen_adult", "mediazen_teen_laugh", "mediazen_adult_laugh", "azure"):
                ds1.append(build_wds(name, mode=mode, cache_size=cache_size, from_mount=from_mount, from_prod=from_prod))
            elif name in ("mediazen", "emilia_yodas_ko", "saltlux_jeju", "saltlux_chungcheong", "saltlux_gyeongsang", "saltlux_jeolla", "saltlux_gangwon"):
                ds5.append(build_wds(name, mode=mode, cache_size=cache_size, from_mount=from_mount, from_prod=from_prod))
            else:
                raise Exception(name)
        if len(ds1 + ds5) == 1:
            # cv_data
            dataset = wds.RoundRobin(ds1+ds5)
        else:
            ds1 = wds.RandomMix(ds1, probs=[100,5,5,50,3,3,25,20,10,1,1], longest=True)
            ds5 = wds.RandomMix(ds5, probs=[1000,700,50,100,200,150,100], longest=True)
            dataset = wds.RandomMix([ds1, ds5], probs=[20, 80], longest=True) # total 100
        # 4) default
        #dataset = []
        #for name in recipe_names:
...
def WebDataset(recipe_names,
    ...
    dataset = WebDataList(recipe_names, mode=mode, cache_size=cache_size, from_mount=from_mount, from_prod=from_prod)
    if gan is True:
        # map partial arg to padding func in gan mode
        data_pipeline[-1] = partial(data_pipeline[-1], gan=gan)
    for func in data_pipeline:
