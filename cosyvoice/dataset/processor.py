...
def padding_lt(data, use_spk_embedding=False, mode='train', gan=False):
    """ Padding the data into training data
    ...
    for sample in data:
        ...
        _speech_feat_emb = []
        for x in speech_feat_emb:
            if len(x) == T_min:
                _speech_feat_emb.append(x)
            elif len(x) < T_min:
                _speech_feat_emb.append(np.pad(x, ((0, T_min-len(x)), (0,0)), 'constant', constant_values=0))
            else:
                t_start = random.randrange(0, len(x) - T_min)
        ...
        if "utt_embedding" in sample[0]:
            utt_embedding = torch.stack([sample[i]['utt_embedding'] for i in order], dim=0)
            batch.update({"utt_embedding": utt_embedding})
        if "spk_embedding" in sample[0]:
            spk_embedding = torch.stack([sample[i]['spk_embedding'] for i in order], dim=0)
            batch.update({"spk_embedding": spk_embedding})
