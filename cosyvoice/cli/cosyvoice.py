...
class CosyVoice2(CosyVoice):
    ...
    def __init__(self, model_dir, load_jit=False, load_trt=False, load_vllm=False, fp16=False, trt_concurrent=1, llm_path=None, flow_path=None):
        ...
        hyper_yaml_path = '{}/cosyvoice2_1.5b.yaml'.format(model_dir)
        if not os.path.exists(hyper_yaml_path):
            raise ValueError('{} not found!'.format(hyper_yaml_path))
        with open(hyper_yaml_path, 'r') as f:
            #configs = load_hyperpyyaml(f, overrides={'qwen_pretrain_path': os.path.join(model_dir, 'CosyVoice-BlankEN')})
            configs = load_hyperpyyaml(f, overrides={'qwen_pretrain_path': "/home/longtou.2024/mount/longtou/saved/cosyvoice/pretrained_models/Qwen2.5-1.5B"})
