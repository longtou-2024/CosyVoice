...
# qpp
prompt_spk_speech_16k = dict()
for audio_path in Path("test/qpp").glob("*.wav"):
    uttid = audio_path.stem
    prompt_spk_speech_16k[uttid] = load_wav(audio_path, 16000)
prompt_spk_speech_24k = dict()
for audio_path in Path("test/qpp").glob("*.wav"):
...
prompt_spk_sent = dict()
for json_path in Path("test/qpp").glob("*.json"):
...
for d_idx, spk_id in enumerate(prompt_spk_sent):
    ...
    for t_idx, item in enumerate(script_2):
        ...
        gen = cosyvoice.inference_zero_shot(item["text"], sh_prompt_sent, sh_prompt_speech_16k, stream=False, text_frontend=False, prompt_speech_24k=sh_prompt_speech_24k)
        ret = next(gen)
