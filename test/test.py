...
for i, j in enumerate(cosyvoice.inference_instruct2('<fast>안녕하세요</fast>, 여러분의 친구, 박의주입니다.', PROMPT_TEMPLATE(spk_id=spk_id, emotion="", pitch="", tone="", infer=True), prompt_speech_16k, stream=False, text_frontend=False)):
    torchaudio.save(f"{outdir}/{fout_names[1]}", j['tts_speech'], cosyvoice.sample_rate)
for i, j in enumerate(cosyvoice.inference_instruct2('<slow>안녕하세요</slow>, 여러분의 친구, 박의주입니다.', PROMPT_TEMPLATE(spk_id=spk_id, emotion="", pitch="", tone="", infer=True), prompt_speech_16k, stream=False, text_frontend=False)):
