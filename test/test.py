...
fout_names = ["fast.wav", "slow.wav", "base.wav"]
...
for i, j in enumerate(cosyvoice.inference_zero_shot2('알수없음', '<fast>안녕하십니까</fast>, <fast>오늘</fast> <fast>오전</fast> <fast>날씨는</fast> <fast>맑고</fast> <fast>오후는</fast> <fast>구름이</fast> <fast>조금</fast> <fast>끼겠습니다.</fast>', azure_sent, prompt_speech_16k, stream=False, text_frontend=False)):
    torchaudio.save(f"{outdir}/{fout_names[0]}", j['tts_speech'], cosyvoice.sample_rate)
for i, j in enumerate(cosyvoice.inference_zero_shot2('알수없음', '<slow>안녕하십니까</slow>, <slow>오늘</slow> <slow>오전</slow> <slow>날씨는</slow> <slow>맑고</slow> <slow>오후는</slow> <slow>구름이</slow> <slow>조금</slow> <slow>끼겠습니다.</slow>', azure_sent, prompt_speech_16k, stream=False, text_frontend=False)):
    torchaudio.save(f"{outdir}/{fout_names[1]}", j['tts_speech'], cosyvoice.sample_rate)
for i, j in enumerate(cosyvoice.inference_zero_shot2('알수없음', '안녕하십니까, 오늘 오전 날씨는 맑고 오후는 구름이 조금 끼겠습니다.', azure_sent, prompt_speech_16k, stream=False, text_frontend=False)):
    torchaudio.save(f"{outdir}/{fout_names[2]}", j['tts_speech'], cosyvoice.sample_rate)

#for i, j in enumerate(cosyvoice.inference_zero_shot2("속삭임", '안녕하십니까, 오늘 오전 날씨는 맑고 오후는 구름이 조금 끼겠습니다.', azure_sent, prompt_speech_16k, stream=False, text_frontend=False)):
#    torchaudio.save(f"{outdir}/{fout_names[2]}", j['tts_speech'], cosyvoice.sample_rate)
#
#for i, j in enumerate(cosyvoice.inference_zero_shot2("알수없음", 'azure<|endofprompt|>안녕하십니까, 오늘 오전 날씨는 맑고 오후는 구름이 조금 끼겠습니다.', azure_sent, prompt_speech_16k, stream=False, text_frontend=False)):
